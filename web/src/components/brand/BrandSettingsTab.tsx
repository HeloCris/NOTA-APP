...
export function BrandSettingsTab({ brand, store, onStoreUpdated }: BrandSettingsTabProps) {
    ...
    const handleReactivate = async () => {
        ...
        try {
            if (!store) {
                const res = await brandService.activateD2C();
                await updateSession({ access: res.data.access, refresh: res.data.refresh });
            ...
            toast.error("Erro ao ativar a loja.");
        } finally {
    ...
    const onSubmit = async (data: StoreFormValues) => {
        ...
        try {
            if (!store) {
                // Activate D2C first if it doesn't exist
                const res = await brandService.activateD2C();
                await updateSession({ access: res.data.access, refresh: res.data.refresh });
            }
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="flex items-end justify-between mb-6 gap-4 flex-wrap">
                <div>
                    <h1 className="text-[27px] font-extrabold text-[#263847] mb-1 tracking-[-0.01em] font-jakarta">Configurações da Marca e Loja</h1>
                    <p className="text-[13px] text-[#5A6067] m-0">Estes dados aparecem na sua vitrine e no cabeçalho do painel.</p>
                </div>
            </div>

            {isStoreDeactivated && store && (
            ...
            {!store && (
                <div className="mb-6 bg-[#F0F5EE] border border-[#B8CFAF] rounded-[16px] p-4 flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-[#5C6B4E]/10 flex items-center justify-center flex-shrink-0">
                        <svg className="w-5 h-5 text-[#3A5C30]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    <div>
                        <h3 className="text-[14.5px] font-bold text-[#3A5C30] font-jakarta">Loja ainda não ativada</h3>
                        <p className="text-[13px] text-[#5C6B4E] mt-0.5">Sua marca está aprovada! Salve seu perfil abaixo para ativar automaticamente sua vitrine.</p>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-[22px] items-start">
                <form 
                    ...
                    className={`bg-white border border-[#EFEBDD] rounded-[20px] shadow-[0_1px_3px_rgba(35,40,45,0.06)] p-[28px_30px] transition-all ${isStoreDeactivated && store ? 'opacity-60 pointer-events-none select-none grayscale-[0.2]' : ''}`}
                >
                    <h3 className="text-[16.5px] font-extrabold mb-1 text-[#23282D]">Perfil da Marca e Loja</h3>
                    <p className="text-[12.5px] text-[#5A6067] mb-[22px]">Nome, identidade visual e bio olfativa que os clientes vão ver.</p>

                    {/* DADOS DA MARCA (Bloqueados - Premium UI) */}
                    <div className="mb-[26px]">
                        <div className="flex items-center gap-2.5 mb-3">
                            <h4 className="text-[12.5px] font-extrabold text-[#23282D] uppercase tracking-[0.08em]">Dados de Registro</h4>
                            <div className="flex items-center gap-1 px-2 py-[3px] rounded bg-[#F0F5EE] border border-[#B8CFAF]">
                                <svg className="w-[10px] h-[10px] text-[#3A5C30]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                </svg>
                                <span className="text-[9px] font-extrabold text-[#3A5C30] uppercase tracking-[0.06em] leading-none">Verificado</span>
                            </div>
                        </div>
                        
                        <div className="bg-[#F8F7F2] border border-[#E6E1D2] rounded-[16px] p-5 shadow-[inset_0_1px_4px_rgba(0,0,0,0.02)] flex flex-col gap-5">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                                <div className="md:col-span-2">
                                    <div className="text-[10px] uppercase tracking-[0.08em] font-extrabold text-[#354B5E] mb-1 flex items-center gap-1.5">
                                        Nome da Marca
                                    </div>
                                    <div className="text-[14.5px] font-semibold text-[#23282D]">{brand?.name || "-"}</div>
                                </div>
                                <div>
                                    <div className="text-[10px] uppercase tracking-[0.08em] font-extrabold text-[#354B5E] mb-1 flex items-center gap-1.5">
                                        CNPJ
                                    </div>
                                    <div className="text-[14.5px] font-semibold text-[#23282D]">{brand?.cnpj ? maskCnpj(brand.cnpj) : "-"}</div>
                                </div>
                                <div>
                                    <div className="text-[10px] uppercase tracking-[0.08em] font-extrabold text-[#354B5E] mb-1 flex items-center gap-1.5">
                                        Registro INPI
                                    </div>
                                    <div className="text-[14.5px] font-semibold text-[#23282D]">{brand?.inpi_registration || "-"}</div>
                                </div>
                            </div>
                            
                            <div className="bg-white/60 border border-[#EFEBDD] rounded-xl p-3.5 flex items-start gap-3 shadow-sm">
                                <svg className="w-[18px] h-[18px] text-[#93927F] flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <p className="text-[12px] text-[#5A6067] leading-[1.5] m-0">
                                    Estes dados foram validados e atrelados ao seu registro oficial. Para solicitar correções ou atualizações em informações sensíveis, <a href="#" className="font-bold text-[#354B5E] hover:underline">entre em contato com o suporte</a>.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-[#F5E7DE] border border-[rgba(168,90,56,0.12)] rounded-[20px] p-[22px_22px_24px]">
                    ...
                    <ul className="m-0 pl-[18px] text-[12px] text-[#5A6067] leading-[1.9] list-disc">
                        ...
                        <li>Os dados da marca (CNPJ, INPI e Nome) são protegidos.</li>
                    ...
                    <div className="bg-white px-[28px] py-[22px] flex items-center justify-between gap-6 flex-wrap">
                        <div>
                            <div className="text-[14px] font-bold text-[#23282D] mb-0.5">
                                {isStoreDeactivated ? 'Reativar loja' : 'Desativar loja'}
                            </div>
                            <div className="text-[12.5px] text-[#5A6067] max-w-[480px] leading-[1.6]">
                                {isStoreDeactivated 
                                    ...
                                    : 'Sua vitrine será removida da plataforma, seus perfumes ficarão ocultos e você não receberá novos pedidos online. O Catálogo Oficial não será afetado.'}
                            </div>
                        </div>
                    ...
                    >
                        ...
                        <div className="px-[32px] pt-[30px] pb-[32px]">
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    <h2 className="font-jakarta text-[21px] font-extrabold text-[#1A1A1A] leading-tight tracking-[-0.01em]">Desativar Loja?</h2>
                                    <p className="text-[13px] text-[#5A6067] leading-[1.7] mt-1.5">
                                        Sua vitrine ficará offline, impedindo novos pedidos de consumidores finais. Você poderá <span className="font-semibold text-[#3A5C30]">reativar</span> sua loja a qualquer momento.
                                    </p>
                                </div>
                            </div>
                                ...
                                <button
                                    ...
                                    onClick={async () => {
                                        if (canDeactivate) {
                                            setIsSubmitting(true);
                                            try {
                                                await brandService.deactivateD2C();
                                                toast.success("Loja foi desativada.");
